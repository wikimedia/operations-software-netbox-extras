...
class Ganeti:
    ...
    def get_groups(self) -> Dict:
        ...
        for group in self._query('groups?bulk=1'):
            ...
            logger.debug('Loaded %d Ganeti nodes for group %s in cluster %s',
                         len(group['node_list']), group['name'], self.url)
...
class NetboxCluster(Netbox):
    ...
    def get_devices(self) -> Dict:
        ...
        logger.debug('Loaded %d Netbox devices in cluster %s (ID %d)',
                     len(devices), self.cluster, self.cluster.id)
        return devices
...
class GanetiNetboxSyncer:
    ...
    def sync_vms(self) -> None:
        ...
        for name, cluster in self.netbox_clusters.items():
            ...
            if name in self.ganeti_groups:
                instances = {key: value for key, value in ganeti_instances.items()
